...
model_tab
write_csv(model_tab, "output/model_selection/diph_ag_logistic_model_selection_table.csv")
...
model_tab <- model_tab %>% select(df:weight) %>% rownames_to_column(var="model") %>% as_tibble()
model_tab
write_csv(model_tab, "output/model_selection/diph_beech_logistic_model_selection_table.csv")
...
lsm.models <- lapply(lsm_formulae, FUN=glmer, data=diph, family='binomial') 
model.list <- model.sel(lsm.models)
model_tab <- as.data.frame(model.list)
model_tab <- model_tab %>% select(df:weight) %>% rownames_to_column(var="model") %>% as_tibble()
model_tab
write_csv(model_tab, "output/model_selection/diph_lsm_logistic_model_selection_table.csv")
