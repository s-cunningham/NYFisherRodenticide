...
vsDataBundle <- list(y=brom, # response
...
var_scale_code <- nimbleCode({
  ...
  for (i in 1:N) {
    ...
    logit(p[i]) <- beta0 + eps[WMUA[i]] + eta[sampleID[i]] +
      z[1]*beta[1]*age[i] +
      z[2]*beta[2]*age2[i] +
      z[3]*beta[3]*sex[i] +
...
plot(samplesIndicator[,'z[3]'], type="l", cex = 0.4, main = "z[3] traceplot")
