...
twn <- spTransform(twn, CRS="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs")
cents <- coordinates(twn)
centid <- twn@data$NAME
muntype <- twn@data$MUNI_TYPE
...
nys_sf <- st_as_sf(nys)
spacing <- 10000
nys_grid <- st_make_grid(nys_sf, cellsize = c(spacing, spacing),
                         what = 'polygons') 
nys_grid <- nys_grid[nys_sf] %>% st_sf('geometry' = ., data.frame('ID' = 1:length(.)))
grid_sp <- as(nys_grid, "Spatial")
...
names(plc)[1] <- "id"
plc$id <- as.character(plc$id)

grid_df <- fortify(grid_sp, region="ID")
grid2 <- left_join(grid_df, plc, by="id")

ggplot() + 
  geom_polygon(data=grid2, aes(x=long, y=lat, group=group, fill=Forest)) 

# Convert SPDF to raster 
grid.c <- coordinates(grid_sp)
grid.c <- cbind(grid.c, plc)
names(grid.c)[1:2] <- c("x","y")
grid.c <- grid.c[,-3]
grid.r <- rasterFromXYZ(grid.c[,c("x","y","Forest")])

fha <- readOGR("data/spatial", "FisherHarvestArea")
fha <- spTransform(fha, CRS="+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs")
fha2 <- fortify(fha, region="Name")

for.ha <- mask(x=grid.r, mask=fha)
for.ha2 <- rasterToPolygons(for.ha, na.rm=TRUE)
